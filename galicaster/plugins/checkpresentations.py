...
# Galicaster 3.x UCT - Drop empty and/or duplicate presentation tracks before ingest
...
def init():
    ...
    try:
        ffprobe_version = subprocess.check_output([ffprobe_bin, '-version'], text=True)
    except (OSError, subprocess.CalledProcessError):
        logger.error('Unable to run ffprobe executable: %s', ffprobe_bin)

    try:
        videomatch_version = subprocess.check_output([videomatch_bin, '-version'], text=True)
    except (OSError, subprocess.CalledProcessError):
    ...
    if (ffprobe_version.split(' ')[0] == 'ffprobe') and (videomatch_version.split(' ')[0] == 'videomatch.pl'):
        ...
        logger.warning('ffprobe and/or videomatch not available: plugin will not run')
...
def drop_presentations(sender, operation_code, mp):
        ...
        if type == flavor_p1 or type == flavor_p2:
            ...
            try:
                ff_bitrate = subprocess.check_output([ffprobe_bin,'-v','error','-show_entries','format=bit_rate','-of',
                   'default=nokey=1:noprint_wrappers=1', t.getURI()], text=True).replace("\n", "")
    ...
    if (removed == False) and (bitrate_p1 > 0) and (bitrate_p2 > 0):
        ...
        if (bitrate_diff > bitrate_diff_match_threshold) and (bitrate_diff < bitrate_diff_threshold):
            ...
            try:
                match_result = subprocess.check_output([videomatch_bin, track_p1.getURI(), track_p2.getURI()], text=True).replace("\n", "")
...
def __track_empty(t):
    ...
    try:
        empty_result = subprocess.check_output([videomatch_bin, '--empty', t.getURI()], text=True).replace("\n", "")
